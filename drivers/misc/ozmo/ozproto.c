...
{
	if (presleep == 0 || presleep > OZ_PRESLEEP)
			pd->presleep = OZ_PRESLEEP * 100;
	else
...
/*
 *Overwrite the presleep value coming from abc123. Nefario is 0x23 anyways.
 ...
 */
	...
	if ((new_apps & 0x1) && (body->session_id)) {
		if (pd->session_id) {
			...
			oz_trace("session_id = %02x mac_address = %pM\n", pd->session_id, pd->mac_addr);
		}
	} else {
		if (pd->session_id && !body->session_id) {
			rsp_status = OZ_STATUS_SESSION_TEARDOWN;
			stop_needed = 1;
			oz_trace("OZ_STATUS_SESSION_TEARDOWN of mac_address = %pM\n", pd->mac_addr);
		} else {
			new_apps &= ~0x1;  /* Resume not permitted */
			pd->session_id =
				oz_get_new_session_id(body->session_id);
